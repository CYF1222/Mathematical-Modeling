...
def solve_direct_ilp():
    ...
    # 设置求解时间限制（秒）
    solver.set_time_limit(300000000)  # 5分钟
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        return solution, total_pieces, all_placements
...
def apply_checkerboard_coloring():
    ...
    for i in range(ROWS):
        for j in range(COLS):
            ...
            color_index = (i + j + k - 1) % C_COLORS + 1
...
def visualize_solution(solution, total_pieces, all_placements):
    ...
    cmap_poly = ListedColormap(colors[1:])
    
    colors_color = plt.cm.viridis(np.linspace(0, 1, 4))
    cmap_color = ListedColormap(colors_color[1:])
    
    # 绘制网格着色
    im1 = ax1.imshow(coloring, cmap=cmap_color, vmin=1, vmax=3)
    ax1.set_title(f'棋盘着色 (C=3)', fontsize=14)
    ax1.set_xlabel('列')
    ax1.set_ylabel('行')
    ...
    cbar1.set_ticks(range(1, 4))
    cbar1.set_ticklabels(['颜色1', '颜色2', '颜色3'])
    ...
    im2 = ax2.imshow(grid, cmap=cmap_poly, vmin=1, vmax=9)
    ax2.set_title(f'骨牌覆盖方案 (总骨牌数: {total_pieces})', fontsize=14)
    ax2.set_xlabel('列')
    ax2.set_ylabel('行')
    
    cbar2 = plt.colorbar(im2, ax=ax2)
    cbar2.set_ticks(range(1, 10))
    cbar2.set_ticklabels([polyomino_names[i] for i in range(1, 10)])
...
def main():
    """主函数"""
    print("=" * 60)
    print("骨牌覆盖优化问题求解 - 直接ILP方法")
    print(f"网格大小: {ROWS} × {COLS} = {ROWS*COLS} 单元格")
    print("=" * 60)
    
    # 求解问题
    solution, total_pieces, all_placements = solve_direct_ilp()
